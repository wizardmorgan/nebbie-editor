...
if (str_contains($path, '/reservations')) {
    ...
    if ($method === 'POST') {
        ...
        if ((int)$body['zone_num'] <= 0) {
            respond(400, ['status' => 'KO', 'reason' => 'zone_num must be > 0']);
        }
        if ((int)$body['start_vnum'] <= 0 || (int)$body['end_vnum'] <= 0) {
            respond(400, ['status' => 'KO', 'reason' => 'start_vnum and end_vnum are required']);
        }
