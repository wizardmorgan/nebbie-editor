...
bool post_reservation_sync(QNetworkAccessManager& network,
    ...
    if (coordinator_url.isEmpty() || coordinator_token.isEmpty()) {
        if (error_out) {
            *error_out = QStringLiteral("coordinator_url or coordinator_token not configured");
        }
        return false;
    }
    if (reservation.zone_num <= 0) {
        if (error_out) {
            *error_out = QStringLiteral("zone_num is required");
        }
        return false;
    }
    if (reservation.end_vnum < reservation.start_vnum) {
        if (error_out) {
            *error_out = QStringLiteral("end_vnum must be >= start_vnum");
        }
        return false;
    }
    if (reservation.kind != "room" && (reservation.start_vnum <= 0 || reservation.end_vnum <= 0)) {
        if (error_out) {
            *error_out = QStringLiteral("start_vnum and end_vnum are required");
        }
        return false;
    }
